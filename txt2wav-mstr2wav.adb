...
procedure mstr2wav(morsestr: string; wpm: integer) is

	volume: constant float := 0.8;
	...
	-- dot								1	dit
	-- dash								3	dah
	-- space after dot or dash		1	eod
	-- space after a letter			3	eol
	-- space after a word			7	eow
...
if iwpm=20 then
	wio.create(fout, wio.out_file, "20wpm.wav");
elsif iwpm=10 then
	wio.create(fout, wio.out_file, "10wpm.wav");
elsif iwpm=15 then
	wio.create(fout, wio.out_file, "15wpm.wav");
elsif iwpm=30 then
	wio.create(fout, wio.out_file, "30wpm.wav");
elsif iwpm=40 then
	wio.create(fout, wio.out_file, "40wpm.wav");

elsif iwpm=25 then
	wio.create(fout, wio.out_file, "25wpm.wav");

elsif iwpm=35 then
	wio.create(fout, wio.out_file, "35wpm.wav");

else
	put_line(" WPM must be 10, 15, 20, 25, 30, 35, or 40 !");
	put_line(" Default is 20.");
	raise program_error;
end if;
...
------- end of header;  now write data ----------------------------
	...
	for k in 0..oni-1 loop
		...
		for i in onn(k+1)..off(k+1)-1 loop
			...
			sample:=volume*sin(twopi*float(i)*frequency/sampleRate);
			bytesFromFloat(sample, b1, b2);
			wio.write(fout,b1);
			wio.write(fout,b2);
		end loop; --i
